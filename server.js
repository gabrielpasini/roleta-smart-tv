...
const commitUpdateToGithub = require("./utils/post-commit");
...
app.post("/api/people", (req, res) => {
  ...
  fs.readFile(DB_PATH, "utf8", (err, data) => {
    ...
    fs.writeFile(DB_PATH, JSON.stringify(db, null, 2), (err) => {
      ...
      commitUpdateToGithub(
        db,
        `Atualização de status para ${name}`,
        "fix/update-person-state"
      )
        .then(() => {
        ...
        .catch((commitError) => {
          ...
          res.status(500).json({
            error:
              "Dados salvos localmente, mas falha ao enviar commit para o GitHub.",
...
app.post("/api/people/delete", (req, res) => {
  ...
  fs.readFile(DB_PATH, "utf8", (err, data) => {
    ...
    const index = db.allPeople.findIndex(
      (p) => p.name.toLowerCase() === name.toLowerCase()
    );
    if (index === -1) {
    ...
    fs.writeFile(DB_PATH, JSON.stringify(db, null, 2), (err) => {
      ...
      commitUpdateToGithub(db, `Remoção de ${name}`, "fix/update-person-state")
        .then(() => {
          console.log("commit enviado");
